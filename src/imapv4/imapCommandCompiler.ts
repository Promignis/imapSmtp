...
export const imapCommandCompiler = (response: IMAPDataResponse, isLogging?: boolean): string => {
    let respParts = [];
    let resp = (response.tag || '') + (response.command ? ' ' + response.command : '')
    let val: string
    let lastType: string

    let walk = (node: any, options?: { subArray?: any }): void => {
        options = options || {};
        ...
        switch (node.type.toUpperCase()) {
            ...
            case 'SECTION':
                ...
                if (node.section) {
                    resp += '[';
                    node.section.forEach((child: any) => walk(child));
                    resp += ']';
                }
                if (node.partial) {
