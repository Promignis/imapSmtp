...
class TokenParser {
    str: string
    options: any
    parent: ParserInstance
    currentNode: Node
    tree: Node
    pos: number
    state: string
    constructor(parent: ParserInstance, startPos: number, str: string, options: any) {
    ...
    getAttributes() {
        ...
        let walk = (node: Node) => {
            ...
            if (!node.closed) {
                let position = this.pos + this.str.length - 1
                throw new Error('Unexpected end of input at position ' + position);
            }
    ...
    createNode(parentNode?: Node, startPos?: number) {
                    ...
                    switch (chr) {
                        ...
                        case '[':
                            ...
                            if (['OK', 'NO', 'BAD', 'BYE', 'PREAUTH'].indexOf(this.parent.command.toUpperCase()) >= 0 && this.currentNode === this.tree) {
                                ...
                                // term, we just specialize that case here.
...
class ParserInstance {
    ...
    getTag() {
        if (!this.tag) {
            this.tag = this.getElement(imapFormalSyntax.tag() + '*+');
        }
        return this.tag;
    }
...
export const imapCommandParser = (command: string, options: ParserOpts): ParsedCommand => {
    ...
    parser.getSpace() // Removes space
    response.command = parser.getCommand()

    // Each UID command variation is treated as a unique command
    if (['UID'].indexOf((response.command || '').toUpperCase()) >= 0) {
        parser.getSpace()
        response.command += ' ' + parser.getElement(imapFormalSyntax.command())
    }

    // Extract whatever comes after a command, ie. arguments  
